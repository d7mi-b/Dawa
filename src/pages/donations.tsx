...
const Donations = () => {
    ...
    const { mutate: addDonation, isLoading } = api.donation.add.useMutation({
        onSuccess: async (e) => {
    ...
    const handelSubmit = (e: React.FormEvent<HTMLFormElement>) => {
        ...
        addDonation({
            medicine, description, image, quantity, expiry_Date: new Date(expiry_Date), city, neighborhood
        })
    }

    return (
        <section className="w-3/5 mx-auto my-4 py-8 min-h-screen max-lg:w-4/5">
            ...
            <section className="mt-4 mb-8">
                <form onSubmit={handelSubmit}>
                    ...
                    <section className="grid grid-cols-2 gap-8 max-sm:grid-cols-1 max-sm:gap-0">
                        <section className="my-4">
                            <label htmlFor="expiry_Date" className="text-[#7D8597] font-semibold text-lg uppercase">تاريخ انتهاء الدواء</label>
                            <input 
                                type='date' name="expiry_Date" 
                                required placeholder="تاريخ انتهاء الدواء"
                                className={`
                                    w-full p-4 border-2 border-[#7D8597] 
                                    bg-transparent my-2 rounded-md
                                    focus-visible:border-[#00B4D8] focus-visible:outline-none
                        ...
                        <section className="my-4">
                            <label htmlFor="quantity" className="text-[#7D8597] font-semibold text-lg uppercase">الكمية</label>
                            <input 
                    ...
                    <section className="grid grid-cols-2 gap-8 max-sm:grid-cols-1 max-sm:gap-0">
                        <section className="my-4">
                            <label htmlFor="city" className="text-[#7D8597] font-semibold text-lg uppercase">المدينة</label>
                            <input 
                                type='text' name="city" 
                        ...
                        <section className="my-4">
                            <label htmlFor="neighborhood" className="text-[#7D8597] font-semibold text-lg uppercase">الحي</label>
                            <input 
