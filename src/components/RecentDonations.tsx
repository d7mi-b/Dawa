...
const RecentDonations = (props: { donations: Donation[] | undefined }) => {
    ...
    return (
        <article className="page">
            <header className=" font-bold text-3xl mt-14 flex">
                ...
                <h1 className="mt-2">أحدث التبرعات</h1>
            </header>
            ...
            <section className="my-5 grid grid-cols-4 gap-8 max-sm:grid-cols-1 max-lg:grid-cols-2">
                {
                    donations?.map(e => {
                        return (
                            <Link 
                                href={`donation/${e.id}`}
                                className="border-2 my-4 text-center hover:border-[#00B4D8] rounded-md shadow p-4" 
                                key={e.id}
                            >
